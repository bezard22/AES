...
class AES:
    ...
    def keySchedule(self, k: BitArray) -> list[BitArray]:
        kis = []
        ki = k
        for _ in range(self.roundLen[k.len] + 1):
            ki = self.transform(ki)
            kis.append(ki)
        return kis

    def S(self, byte: BitArray) -> BitArray:
        pass
    ...
    def byteSubstitution(self, z: BitArray) -> BitArray:
        zBytes = [z[i*8:(i+1)*8] for i in range(16)]
        zBytes = [self.S(byte) for byte in zBytes]
        return sum(zBytes)
        

    def shiftRows(self, z: BitArray) -> BitArray:
        zBytes = [z[i*8:(i+1)*8] for i in range(16)]
        zBytes = [zBytes[self.permutations["shiftRows"][i]] for i in range(16)]
        return zBytes
    ...
    def aesRound(self, z: BitArray, ki: BitArray) -> BitArray:
        z = self.byteSubstitution(z)
        z = self.shiftRows(z)
        z = self.mixColumns(z)
        z = self.keyAddition(z, ki)
        return z
    ...
    def encrypt(self, x: BitArray, k: BitArray) -> BitArray:
        ...
        z = self.keyAddition(x, kis.pop(0))
        for _ in range(self.roundLen(k.len)):
            z = self.aesRound(z, kis.pop(0))
        self.data["y"] = z.hex
        return z
